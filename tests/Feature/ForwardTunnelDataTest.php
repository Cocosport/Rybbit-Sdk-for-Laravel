...
it('forwards the request with the given method, data, and headers', function () {
    ...
    (new ForwardTunnelData('https://app.rybbit.io/api/track', 'POST', ['event' => 'pageview'], ['X-Real-IP' => '1.2.3.4']))->handle();
...
it('does not throw when the upstream request fails and throw_on_error is disabled', function () {
    ...
    Http::fake(['https://app.rybbit.io/api/track' => Http::response('upstream error', 500)]);

    (new ForwardTunnelData('https://app.rybbit.io/api/track', 'POST', [], []))->handle();
})->throwsNoExceptions();
