...
5. Name publish tags with the `rybbit-sdk-for-laravel-*` convention so consumers can target individual resource groups.
...
- Add a publishable migration: place the migration in `database/migrations`, wire it through a console-guarded `publishesMigrations` call with a `rybbit-sdk-for-laravel-migrations` tag, and test publish behavior with Testbench.
- Wire a new publish tag by adding a `publishes` map inside the existing console-guarded publishing method and naming the tag with `rybbit-sdk-for-laravel-*`.
